...
/**
 * Auto-generated AdminUser Routes
 *
...
import type { AdminUserHooks } from "./admin-user.hooks"
...
/**
 * Set hooks for AdminUser routes
 */
export function setAdminUserHooks(h: AdminUserHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["username", "email", "passwordHash", "role", "createdAt"] as const
...
/**
 * Create AdminUser CRUD routes
 */
export function createAdminUserRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AdminUser not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AdminUser] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[AdminUser] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[AdminUser] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AdminUser] Delete error:", error)
