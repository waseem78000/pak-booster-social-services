...
import type { DepositHooks } from "./deposit.hooks"
...
/**
 * Set hooks for Deposit routes
 */
export function setDepositHooks(h: DepositHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "amount", "method", "transactionId", "senderInfo", "screenshot", "status", "reviewedBy", "reviewedAt", "createdAt"] as const
const RELATION_FIELDS = ["user", "walletTransactions"] as const
...
export function createDepositRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.deposit.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Deposit not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.deposit.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.deposit.update({
        where: { id },
        data: picked.data,
      })
