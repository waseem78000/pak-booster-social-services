...
/**
 * Auto-generated PaymentSettings Routes
 *
...
import type { PaymentSettingsHooks } from "./payment-settings.hooks"
...
let hooks: PaymentSettingsHooks = {}
...
/**
 * Set hooks for PaymentSettings routes
 */
export function setPaymentSettingsHooks(h: PaymentSettingsHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["jazzcashEnabled", "jazzcashNumber", "jazzcashTitle", "easypaisaEnabled", "easypaisaNumber", "easypaisaTitle", "qrEnabled", "qrCode", "qrTitle", "updatedAt"] as const
...
/**
 * Create PaymentSettings CRUD routes
 */
export function createPaymentSettingsRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.paymentSettings.count({ where }),
      ])
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.paymentSettings.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "PaymentSettings not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.paymentSettings.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.paymentSettings.update({
        where: { id },
        data: picked.data,
      })
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[PaymentSettings] Delete error:", error)
