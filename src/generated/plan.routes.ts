...
import type { PlanHooks } from "./plan.hooks"
...
/**
 * Set hooks for Plan routes
 */
export function setPlanHooks(h: PlanHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "description", "platform", "price", "duration", "views", "subscribers", "features", "status", "createdAt"] as const
const RELATION_FIELDS = ["userPlans"] as const
...
/**
 * Create Plan CRUD routes
 */
export function createPlanRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Plan not found" } }, 404)
      }
