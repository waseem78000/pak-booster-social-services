...
/**
 * Auto-generated SiteSettings Routes
 *
...
import type { SiteSettingsHooks } from "./site-settings.hooks"
...
/**
 * Set hooks for SiteSettings routes
 */
export function setSiteSettingsHooks(h: SiteSettingsHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["siteName", "siteTagline", "adminName", "adminPhone", "adminEmail", "adminWhatsapp", "instagram", "youtube", "facebook", "twitter", "telegram", "updatedAt"] as const
...
/**
 * Create SiteSettings CRUD routes
 */
export function createSiteSettingsRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.siteSettings.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[SiteSettings] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "SiteSettings not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.siteSettings.create({
        data: picked.data,
      })
      ...
      console.error("[SiteSettings] Create error:", error)
