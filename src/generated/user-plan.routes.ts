...
/**
 * Auto-generated UserPlan Routes
 *
...
import type { UserPlanHooks } from "./user-plan.hooks"
...
/**
 * Set hooks for UserPlan routes
 */
export function setUserPlanHooks(h: UserPlanHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "planId", "status", "startDate", "endDate", "createdAt"] as const
const RELATION_FIELDS = ["user", "plan"] as const
...
/**
 * Create UserPlan CRUD routes
 */
export function createUserPlanRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[UserPlan] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.userPlan.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "UserPlan not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[UserPlan] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.userPlan.create({
        data: picked.data,
      })
      ...
      console.error("[UserPlan] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.userPlan.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[UserPlan] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[UserPlan] Delete error:", error)
