...
/**
 * Auto-generated WalletTransaction Routes
 *
...
import type { WalletTransactionHooks } from "./wallet-transaction.hooks"
...
let hooks: WalletTransactionHooks = {}
...
/**
 * Set hooks for WalletTransaction routes
 */
export function setWalletTransactionHooks(h: WalletTransactionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "type", "amount", "previousBalance", "newBalance", "description", "orderId", "depositId", "status", "createdAt"] as const
const RELATION_FIELDS = ["user", "order", "deposit"] as const
...
/**
 * Create WalletTransaction CRUD routes
 */
export function createWalletTransactionRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.walletTransaction.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "WalletTransaction not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[WalletTransaction] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.walletTransaction.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.walletTransaction.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[WalletTransaction] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[WalletTransaction] Delete error:", error)
